...
fn manifest_parses_alerting_severity_hints() {
    ...
    let dir = std::env::temp_dir().join(format!("opencapx-manifest-hints-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
    std::fs::write(
        dir.join("opencapx-plugin.json"),
...
fn revocation_disables_blocks_and_reopens() {
    ...
    let index: crate::core::registry::RegistryIndex = serde_json::from_value(serde_json::json!({
        "schemaVersion": 2,
        "generatedAt": 1,
        "publishers": [],
...
fn settings_p1_radio_group_like_dropdown() {
    assert!(decl_err(serde_json::json!([{"key": "a", "type": "radio-group"}])).contains("options"));
    assert!(decl_err(serde_json::json!([
...
fn settings_p1_set_setting_value_enforces_validate() {
    ...
    assert_eq!(
        PluginManager::set_setting_value(&id, "title", &serde_json::json!("BadValue")).unwrap_err(),
        "invalid: pattern"
    );
    assert_eq!(
    ...
    PluginManager::set_setting_value(&id, "store_path", &serde_json::json!("notes.json")).unwrap();
...
fn registry_fixture_full_chain_e2e() {
    ...
    let signed2 =
        crate::core::registry::sign_index(idx2_raw.to_string().as_bytes(), &seed, key_id).unwrap();
    std::env::set_var("OPENCAPX_REGISTRY_OFFICIAL_KEYS", &official_env);
...
fn capability_timeout_secs_bounds() {
    ...
    for bad in [0u32, 601] {
        let err = PluginManager::validate_manifest(&manifest_with_timeout(Some(bad))).unwrap_err();
...
fn sandbox_declaration_validation() {
    ...
    .is_ok());
    let err =
        PluginManager::validate_manifest(&m_with_sandbox(serde_json::json!({"network": "host"})))
            .unwrap_err();
...
fn settings_list_op_guards() {
    ...
    crate::core::set_shared_store(store.clone());
    let err = PluginManager::invoke_setting_list_op("com.x.list", "nope", "list", None, None, None)
        .unwrap_err();
...
fn settings_list_ops_round_trip() {
                ...
                None => match (self.written.as_ref(), std::fs::read(&self.path)) {
                    (Some(written), Ok(current)) if &current == written => {
                        if let Err(e) = std::fs::remove_file(&self.path) {
                            eprintln!("ConfigSnapshot: remove {} failed: {e}", self.path.display());
                        }
                    }
    ...
    let pid_before = pid_of(&id).expect("plugin process must be running before the rejected op");
