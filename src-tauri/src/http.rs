...
mod tests {
    ...
    fn session_start_reply_gates_on_host_and_event() {
        ...
        assert!(
            session_start_reply("cursor", r#"{"hook_event_name":"SessionStart"}"#, &bus).is_none()
        );
        assert!(
            session_start_reply("gemini", r#"{"hook_event_name":"SessionStart"}"#, &bus).is_none()
        );
        // unknown host
        assert!(
            session_start_reply("some-agent", r#"{"hook_event_name":"SessionStart"}"#, &bus)
                .is_none()
        );
        // malformed body
